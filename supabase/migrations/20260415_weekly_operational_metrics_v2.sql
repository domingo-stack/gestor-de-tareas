-- ============================================================
-- RPC v2: get_weekly_operational_metrics
-- Reemplaza los "Próximamente" con data real de growth_events
-- (workflow GRW_Sync_Mixpanel_Events ya corriendo).
-- ============================================================
...
BEGIN
  ...
  FOR i IN REVERSE (p_weeks - 1)..0 LOOP
    ...
    -- Documentos creados / día (promedio de la semana)
    ...
    -- Documentos por usuario activo = docs_dia / DAU
    IF t_dau > 0 THEN
    ...
    ---------- VENTAS ----------
    -- Visitas a checkout (Pago > Intento, total semana)
    ...
    -- Pagos exitosos para calcular tasa de abandono
    SELECT COALESCE(SUM(total_count), 0) INTO t_pago_exito
    FROM growth_events
    WHERE event_name = 'Pago > Exito'
      AND event_date BETWEEN w_start AND w_end;

    -- Tasa de abandono = (1 - exitos/intentos) × 100
  ...
  v_funnel_metrics := jsonb_build_array(
    jsonb_build_object('key', 'sessions_califica', 'label', 'Sesiones web (califica.ai)', 'format', 'number', 'values', v_sessions_vals, 'status', 'ok', 'source', 'mkt_web_metrics (GA4, hostname=califica.ai)'),
    jsonb_build_object('key', 'ctr_cta_registrate', 'label', 'CTR en CTA "Regístrate Gratis"', 'format', 'pct', 'values', v_ctr_cta_vals, 'status', 'ok', 'source', 'page_views(/registro en app) ÷ sessions(califica.ai) × 100'),
    jsonb_build_object('key', 'registros_nuevos', 'label', 'Registros nuevos (sign-ups)', 'format', 'number', 'values', v_registros_vals, 'status', 'ok', 'source', 'growth_users.created_date'),
  ...
  v_product_metrics := jsonb_build_array(
    jsonb_build_object('key', 'dau', 'label', 'Usuarios activos diarios (DAU)', 'format', 'number', 'values', v_dau_vals, 'status', CASE WHEN v_dau_stale THEN 'stale' ELSE 'ok' END, 'source', 'growth_metrics_daily (Mixpanel, promedio semanal)', 'stale_since', v_dau_max_date),
    jsonb_build_object('key', 'docs_creados_dia', 'label', 'Documentos creados / día', 'format', 'number', 'values', v_docs_creados_vals, 'status', CASE WHEN v_events_stale THEN 'stale' ELSE 'ok' END, 'source', 'growth_events: IA > Generar Material (agregado, ÷7 día)'),
    jsonb_build_object('key', 'docs_por_usuario', 'label', 'Documentos creados / usuario activo', 'format', 'number', 'values', v_docs_por_usuario_vals, 'status', CASE WHEN v_dau_stale OR v_events_stale THEN 'stale' ELSE 'ok' END, 'source', 'docs_creados_día ÷ DAU'),
    jsonb_build_object('key', 'descargas_dia', 'label', 'Descargas de documentos / día', 'format', 'number', 'values', v_descargas_vals, 'status', CASE WHEN v_events_stale THEN 'stale' ELSE 'ok' END, 'source', 'growth_events: Doc > Descargar (÷7 día)'),
    jsonb_build_object('key', 'kalichat_dia', 'label', 'Mensajes Kalichat / día', 'format', 'number', 'values', v_kalichat_vals, 'status', CASE WHEN v_events_stale THEN 'stale' ELSE 'ok' END, 'source', 'growth_events: IA > Generar Material :: Kalichat (÷7 día)'),
    jsonb_build_object('key', 'paywall_views', 'label', 'Vistas del Paywall', 'format', 'number', 'values', v_paywall_vals, 'status', CASE WHEN v_events_stale THEN 'stale' ELSE 'ok' END, 'source', 'growth_events: Pago > Paywall visto (total semana)')
  );

  v_sales_metrics := jsonb_build_array(
    jsonb_build_object('key', 'checkout_views', 'label', 'Visitas a página de pago (checkout)', 'format', 'number', 'values', v_checkout_vals, 'status', CASE WHEN v_events_stale THEN 'stale' ELSE 'ok' END, 'source', 'growth_events: Pago > Intento (total semana)'),
    jsonb_build_object('key', 'revenue_diario', 'label', 'Revenue diario (USD, promedio semanal)', 'format', 'usd', 'values', v_revenue_vals, 'status', 'ok', 'source', 'rev_orders.amount_usd ÷ 7'),
    jsonb_build_object('key', 'abandono_checkout', 'label', 'Tasa de abandono de checkout', 'format', 'pct', 'values', v_abandono_vals, 'status', CASE WHEN v_events_stale THEN 'stale' ELSE 'ok' END, 'source', '(1 - Pago Exito ÷ Pago Intento) × 100'),
    jsonb_build_object('key', 'pagos_fallidos', 'label', 'Pagos fallidos / rechazados', 'format', 'number', 'values', v_failed_vals, 'status', 'ok', 'source', 'payment_failed.fecha_pago_fallido'),
    jsonb_build_object('key', 'ticket_promedio', 'label', 'Ticket promedio efectivo', 'format', 'usd', 'values', v_ticket_vals, 'status', 'ok', 'source', 'rev_orders: SUM(amount_usd) ÷ COUNT(*)')
  );
