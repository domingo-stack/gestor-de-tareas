-- ============================================================
-- RPC v5: get_weekly_operational_metrics
-- - Kalichat ahora también tiene country real (workflow v3 con where filter)
-- - Solo DAU + Sessions GA4 + CTR siguen globales
-- ============================================================
...
BEGIN
  ...
  FOR i IN REVERSE (p_weeks - 1)..0 LOOP
    ...
    ---------- PRODUCTO ----------
    -- DAU: SIGUE GLOBAL (Mixpanel Metrics no split por país)
    ...
    -- Docs creados: filtrable por país
    ...
    -- Kalichat: AHORA filtrable por país (workflow v3 con where filter)
  ...
  v_funnel_metrics := jsonb_build_array(
    jsonb_build_object('key', 'sessions_califica', 'label', 'Sesiones web (califica.ai)' || v_country_suffix, 'format', 'number', 'values', v_sessions_vals, 'status', 'ok', 'source', 'mkt_web_metrics (GA4, no filtra por país)'),
    ...
    jsonb_build_object('key', 'registros_nuevos', 'label', 'Registros nuevos (sign-ups)', 'format', 'number', 'values', v_registros_vals, 'status', 'ok', 'source', 'growth_users.created_date — filtrable por país'),
  ...
  v_product_metrics := jsonb_build_array(
    ...
    jsonb_build_object('key', 'kalichat_dia', 'label', 'Mensajes Kalichat / día', 'format', 'number', 'values', v_kalichat_vals, 'status', CASE WHEN v_events_stale THEN 'stale' ELSE 'ok' END, 'source', 'growth_events: Kalichat (÷7) — filtrable por país (where=tipo_material==Kalichat)'),
