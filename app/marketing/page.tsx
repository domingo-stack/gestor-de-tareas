...
import { useCurrencyToggle } from '@/components/marketing/shared/useCurrencyToggle';
...
export default function MarketingPage() {
  ...
  const renderTab = () => {
    switch (activeTab) {
      case 'resumen': return <ResumenTab range={range} currency={currency} />;
      case 'ads': return <AdsOverview range={range} currency={currency} />;
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              {(activeTab === 'ads' || activeTab === 'resumen') && (
                <div className="flex items-center bg-gray-100 rounded-lg p-0.5">
                  {(['PEN', 'USD'] as const).map(m => (
                    <button
                      key={m}
                      onClick={() => currency.setMode(m)}
